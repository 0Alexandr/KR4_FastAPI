...
### `ModuleNotFoundError` при запуске uvicorn

Убедитесь, что запускаете команды **из корня проекта** (`fastapi_kr4/`), а не из папки задания:

```bash
# Правильно (из корня):
uvicorn task_9_1.main:app --reload

# Неправильно (из папки задания):
cd task_9_1
uvicorn main:app --reload  # не найдёт импорты
```
...
### Тесты pytest падают с `RuntimeError: no running event loop`

Убедитесь, что в `pytest.ini` указано:
```ini
[pytest]
asyncio_mode = auto
```

### PowerShell не разрешает активацию venv
